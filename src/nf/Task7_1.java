...
public class Task7_1 {
    ...
    //решение
    private static double F(double x, double alpha) {
        return 1.0/(x*x+alpha)-1.0/(alpha+1);
    }
    //полином Якоби считаем
    private static double P(double x, double i, int k) {
        if (k==0) {
            if (i==0)
                return 1;
            if (i == 1)
    ...
    //сетка
    private static double w( double x, double i, int k) { //функциональная сетка, k-порядок производной
        if (k==0)
    ...
    private static double LL(int i, double x,double alpha)
    {
        return 1*w(x,i,2)-(x*x+alpha)*w(x,i,1)-2*x*w(x,i,0);
    }

    //вычисление интегралов для метода Галеркина
    private static double MyFunction(double x, int i, int j,double alpha, int who)
    {
        if (who==1)
    ...
    private static double SimpsonIntegration(int n, int i,int j, double alpha,int who) //для поиска коэффициентов матриц
    {
        int a=-1;
        int b=1;
        n=100;
    ...
    private static void Galyorkin_method(int n, double alpha,double y[]) {
